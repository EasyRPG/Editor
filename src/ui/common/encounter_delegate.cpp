...
QWidget *QEncounterDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
	...
	for (size_t i = 0; i < mCore->project()->database().troops.size(); i++)
		editor->addItem(QString::fromStdString(mCore->project()->database().troops[i].name));
	return editor;
}
...
void QEncounterDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const
{
	...
	model->setData(index, QString::fromStdString(mCore->project()->database().troops[static_cast<size_t>(id)-1].name));
}
